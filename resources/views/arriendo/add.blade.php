...
@section('content')
        ...
        <div class="col-md-10 col-md-offset-1">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                        ...
                        @foreach($juegos as $juego)
                            <tr>
                                ...
                                <td>
                                    ...
                                    <button class="btn btn-success" onclick="add('{{$juego->id}}','{{$id}}'),this.disabled=true">Añadir al arriendo</button>
                                    <button class="btn btn-danger" onclick="remove('{{$juego->id}}','{{$id}}'),this.disabled=true">Quitar</button>
                 ...
                 <a  href="{!! URL::to('show_arriendo/'.$id)!!}" class="btn btn-success" aria-label="Left Align"role="button">
            <span class="glyphicon glyphicon-ok" aria-hidden="true"></span>Finalizar </a>
                
                                      {!! Form::open(['route' => ['cancelar', $id], 'method' => 'get']) !!}
    ...
    function add(id_juego, id_arriendo){
        $.ajax({
            type:"POST",
            url:"{!! \Illuminate\Support\Facades\URL::to('addarriendo') !!}",
            data:{"id_juego":id_juego ,
        ...
        function remove(id_juego, id_arriendo){
            $.ajax({
                type:"POST",
                url:"{!! \Illuminate\Support\Facades\URL::to('removearriendo') !!}",
                data:{"id_juego":id_juego ,
                    "id_arriendo":id_arriendo
                }
            });
